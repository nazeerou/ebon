...
export const useReadingStore = defineStore('reading', {
  ...
  actions: {
    ...
    // 👇 NEW: Fetch the latest reading BEFORE a given date for a specific machine
    async fetchPreviousReading(machineId, beforeDate) {
      ...
      try {
        const params = {
          machine_id: machineId,
          per_page: 1,
          // Assume API supports filtering by date less than or equal to beforeDate
          // If your API uses a different parameter name, adjust accordingly.
          before_date: beforeDate, // or reading_date_before, or date_less_than
        ...
        if (!response.data.success) {
          // No reading found before that date is not an error; we return null
          if (response.status === 404) {
            return null
          }
          throw new Error(response.data.message || 'Failed to fetch previous reading')
        }

        const readingData = response.data.data
        if (!readingData || !readingData.data || readingData.data.length === 0) {
          return null
        }

        // The API likely returns paginated data; we take the first item
        const previous = readingData.data[0] || readingData
        return previous
      } catch (error) {
        // If the server returns 404 or empty, it's fine
    ...
    async fetchReading(id) {
      ...
      try {
        ...
        if (response.data != null) {
          // Data inaweza kuwa katika response.data.data au moja kwa moja response.data
